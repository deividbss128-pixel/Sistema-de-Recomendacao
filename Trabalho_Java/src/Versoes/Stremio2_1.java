...
public class Stremio2_1 {
    public static void main(String[] args) throws IOException {
        // BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        BufferedReader br = Files.newBufferedReader(Paths.get("Trabalho1/src/teste.csv"));
        StringTokenizer st;
                ...
                case "DIRETORES_COMUNS":
                    ...
                    if (avaliacoesID1 != null && avaliacoesID2 != null) {
                        ...
                        for (Avaliacao avaliacao : avaliacoesID1) {
                            // pego o nome dos diretores de filmes que id1 analizou sem repeticação
                            diretores.add(filmes.get(avaliacao.id_filme).diretor);
                        }
                        Set<String> comuns = new TreeSet<>(); // TreeSet já mantém ordem alfabética
                        for (Avaliacao avaliacao : avaliacoesID2) {
                            String dir = filmes.get(avaliacao.id_filme).diretor;
                            if (diretores.contains(dir))
                                comuns.add(dir);
                        }
                        for (String d : comuns)
